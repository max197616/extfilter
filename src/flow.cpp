...
Poco::NotificationQueue FreeFITask::queue;

FreeFITask::FreeFITask():
	Task("SenderTask"),
	_logger(Poco::Logger::get("FreeFITask"))
...
void FreeFITask::runTask()
{
	_logger.debug("Starting FreeFITask...");
	...
	while(!isCancelled())
	{
		...
		if (pNf)
		{
			FreeFINotification::Ptr nf = pNf.cast<FreeFINotification>();
			if (nf)
			{
				ndpi_free_flow(nf->getNDPIFlow());
				free(nf->getSrcId());
				free(nf->getDstId());
			}
		}
	...
	_logger.debug("Stopping FreeFITask...");
}
